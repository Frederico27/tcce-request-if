            ...
            <header class="bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8 py-4 shadow-sm sticky top-0 z-10">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full">
                    <div class="flex justify-between items-center w-full">
                        <a href="{{ route('transactions.manager.index') }}" wire:navigate class="flex-shrink-0">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                ...
                                                @if ($transaction->action == 'return')
                                                    <button
                                                        ...
                                                        <a wire:navigate
                                                            href="{{ route('transactions.manager.return.view', $transaction->id_transactions) }}">View</a>
                                                    </button>
                                                @else
                                                    <button
                                                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded mr-1">
                                                        <a wire:navigate
                                                            href="{{ route('transactions.manager.view', $transaction->id_transactions) }}">View</a>
                                                    </button>
                                                @endif

                                                <!-- Approve button - only shown for pending transactions -->
                                                @if ($transaction->status == 'pending' && $transaction->action == 'request')
                                                    <button wire:click="approveConfirmation('{{ $transaction->id_transactions }}')"
                                                        class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded mr-1">
                                                        Approve
                                                    </button>
                                                    <button wire:click="$set('showRejectModal', true); $set('rejectTransactionId', '{{ $transaction->id_transactions }}')"
